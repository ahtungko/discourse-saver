...
通用 Discourse 论坛内容保存工具 - 一键保存任意 Discourse 论坛（如 LinuxDo、Discourse Meta、Rust Users 等数百个站点）的帖子和评论到 Obsidian、飞书多维表格、Notion、语雀、思源笔记，或导出为 HTML 文件。

支持 **Chrome 扩展** 和 **油猴脚本（Tampermonkey）** 两种安装方式。

> **V5.4.2 更新**（2026-04-08）：
> - **悬浮按钮（FAB）** - 页面右上角悬浮按钮，可自由拖动，不再拦截链接按钮
> - **单击保存整帖** - 点击悬浮按钮直接保存当前帖子到已启用的平台
> - **长按选择楼层** - 长按悬浮按钮弹出菜单，输入楼层号保存指定楼层评论
> - **多楼层合并保存** - 支持范围（2-8）、列表（1,3,5）、混合（1-5,8,10-12）输入，合并成一个文件
> - **API 抓取楼层** - 通过 Discourse API 获取指定楼层，彻底解决懒加载问题
> - **全平台支持** - 多楼层逻辑适用于 OB/飞书/Notion/语雀/思源/HTML 所有平台
>
> **V5.3.1 更新**（2026-04-02）：
> - **飞书上传选项独立控制** - 正文/MD附件/HTML附件三项独立勾选
> - **全面审计修复14项bug** - 静默失败、缓存导致卡死、警告未显示等
> - **详细日志增强** - 保存路径、文件名、文件大小全部记录
...
| 操作 | 效果 |
|-----|------|
| **单击** 悬浮按钮（FAB） | 保存整个帖子到已启用的平台（Obsidian/飞书/Notion/语雀/思源笔记/HTML） |
| **长按** 悬浮按钮（800ms） | 弹出楼层菜单，可输入楼层号保存指定评论 |
| **楼层输入格式** | 单楼层：`5`；范围：`2-8`；列表：`1,3,5`；混合：`1-5,8,10-12` |
| **多楼层结果** | 合并为一个文件（`标题-2至8楼.md`），不生成多个文件 |
...
| **五平台保存** | Obsidian / 飞书 / Notion / 语雀 / 思源 / HTML 导出 |
| **评论保存** | 支持折叠模式和用户名超链接 |
...
2. 插件会**自动检测**并激活，页面右上角出现**悬浮按钮**（圆形图标）
3. **单击悬浮按钮** → 保存整个帖子到已启用的平台
4. **长按悬浮按钮（800ms）** → 弹出楼层菜单
5. 在楼层菜单中输入楼层号（如 `3`、`2-8`、`1,3,5`、`1-5,8`）
6. 点击「保存指定楼层」→ 指定楼层评论合并保存为一个文件

> 悬浮按钮可以拖动到页面任意位置。不影响论坛原始链接按钮的功能。
...
**A:** 请检查：
1. Obsidian 是否已运行（保存到 OB 时必须）
2. 扩展设置中是否至少启用了一个保存平台
3. 浏览器是否允许 `obsidian://` 协议（首次使用会弹窗询问）
4. 按 F12 查看控制台是否有错误
...
**A:** 可以直接拖动悬浮按钮到不影响阅读的位置（如页面角落）。如果需要完全关闭悬浮按钮，在设置中关闭「启用插件」开关，然后刷新页面。
...
**A:** 不冲突！所有保存目标（Obsidian、飞书、Notion、语雀、思源笔记、HTML 导出）完全独立。你可以同时启用所有平台，一键保存到多个地方。任何一个平台保存失败不会影响其他平台。
...
- **新增**：悬浮按钮（FAB）- 页面右上角悬浮圆形按钮，可自由拖动
  - 不再拦截论坛原始链接按钮，彻底解决误触发问题
  - 单击 → 保存整个帖子到所有已启用平台
  - 长按（800ms）→ 弹出楼层选择菜单
- **新增**：多楼层保存功能
  - 楼层输入支持：单个（`5`）、范围（`2-8`）、列表（`1,3,5`）、混合（`1-5,8,10-12`）
  - 多楼层合并为一个文件，文件名自动生成（`标题-2至8楼`）
  - 通过 Discourse API 获取指定楼层，彻底解决懒加载导致楼层缺失的问题
  - 适用于所有平台：Obsidian / 飞书 / Notion / 语雀 / 思源笔记 / HTML
- **修复**：思源笔记保存缺少分类和标签字段
- **修复**：飞书 HTML 导出缺少分类和标签
- **修复**：`extractContent()` 失败时增加3次重试机制（500/1000/1500ms）
